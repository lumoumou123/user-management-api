...
async function handler(event, context) {
    ...
    try {
        ...
        // 检查是否有API密钥 - POST和PUT请求需要API密钥验证
        if ((httpMethod === 'POST' || (httpMethod === 'PUT' && pathParameters.partition_key && pathParameters.sort_key))) {
            const apiKey = event.headers['x-api-key']; // 从请求头获取API密钥
            if (!apiKey) {
                return {
                    statusCode: 403,
                    headers: getCorsHeaders(),
                    body: JSON.stringify({ message: '缺少API密钥，访问被拒绝' })
                };
            }
            // API Gateway已经验证了API密钥，这里只需检查其存在性
        }
        // 路由请求到相应的处理函数
        // 1. 创建项目的POST请求
        if (httpMethod === 'POST') {
 ...
 * @returns HTTP响应
 */
async function getItems(partition_key, filter) {
    // 构建查询参数
    const params = {
        TableName: TABLE_NAME,
 ...
 * @returns HTTP响应
 */
async function updateItem(keys, data) {
    // 验证是否有要更新的字段
 ...
 * @returns HTTP响应
 */
async function getTranslation(keys, language) {
    if (!language) {
        return {
            statusCode: 400,
            headers: getCorsHeaders(),
    ...
    try {
        ...
        // 缓存翻译结果以备将来使用
        await cacheTranslation(item, translateResult.TranslatedText || '', language, translateResult.SourceLanguageCode || 'en');
        // 返回翻译结果
        return {
            statusCode: 200,
            headers: getCorsHeaders(),
 ...
 */
async function checkCachedTranslation(partitionKey, sortKey, language) {
    // 首先获取原始项目以获取其描述
 ...
 */
async function cacheTranslation(originalItem, translatedText, targetLanguage, sourceLanguage) {
    // 构建缓存键
